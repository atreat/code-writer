...
- CodeWriter branding: [`SPECs/codewriter-branding-spec.md`](SPECs/codewriter-branding-spec.md) — rename user-facing app branding while preserving fork-compatible identifiers, paths, and the legacy `writer` shell command. (99e520d)
- Image preview zoom input: [`SPECs/image-preview-zoom-input-spec.md`](SPECs/image-preview-zoom-input-spec.md) — make the image preview percentage an editable field for precise zoom adjustments. (cd7b2a6)
- Image preview control stacking: [`SPECs/image-preview-control-stacking-spec.md`](SPECs/image-preview-control-stacking-spec.md) — keep the image control dock below the floating tab chrome so it remains visible and clickable. (3b6f76a)
- Image preview first-load and pane-wide controls: [`SPECs/image-preview-first-load-and-controls-spec.md`](SPECs/image-preview-first-load-and-controls-spec.md) — make the first image load resolve reliably, including cached/complete assets, and keep controls available while hovering the whole preview pane. (580c802)
- Source editor tab-bar blur: [`SPECs/source-editor-tab-bar-blur-spec.md`](SPECs/source-editor-tab-bar-blur-spec.md) — apply the Markdown editor's progressive top blur to code panes so scrolling source text remains legible beneath the floating tabs. (4267318)
- Source editor top inset and image preview loading lifecycle: [`SPECs/source-editor-top-inset-and-image-loading-spec.md`](SPECs/source-editor-top-inset-and-image-loading-spec.md) — keep source line 1 below the floating tab bar, crossfade image previews after their visible asset loads, and release inactive preview resources after a bounded delay. (7b9f7e7)
- Image preview trackpad scrolling: [`SPECs/image-preview-editor-spec.md`](SPECs/image-preview-editor-spec.md) — make zoomed images navigable with natural two-finger trackpad scrolling, bounded pan, and gesture-safe wheel behavior.
- Audio/video media preview: [`SPECs/media-preview-editor-spec.md`](SPECs/media-preview-editor-spec.md) — add first-class audio and video discovery/opening with Writer surfaces around native WebKit playback controls, common-format associations, metadata-only loading, and watcher-safe reload behavior.
- Image preview editor: [`SPECs/image-preview-editor-spec.md`](SPECs/image-preview-editor-spec.md) — add first-class image file discovery/opening plus a read-only preview tab with fit/zoom/pan, macOS trackpad pinch zoom, and watcher-safe reload behavior.
- Lightweight source-code mode: [`SPECs/code-editor-implementation-spec.md`](SPECs/code-editor-implementation-spec.md) — extend the workspace model, sidebar/search/recents/open flows, and watcher index to allowlisted source-text files; add a lazy CodeMirror source editor; save source text raw with line-ending, permission, and backend conflict safeguards; and reject unsupported, binary, and oversized files before editable load.
- Code editor implementation docs: [`SPECs/code-editor-implementation-docs-spec.md`](SPECs/code-editor-implementation-docs-spec.md) - document the full lightweight source-code viewing/editing plan under `docs/code-editor/`, centered on a kind-based workspace file model, a parallel source editor surface, raw-text save safety, performance/CSP constraints, and existing Vite+/Rust/WebdriverIO validation.
