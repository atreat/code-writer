...
5. Sign `CodeWriter.app` and the DMG with the Developer ID identity from `.env`.
...
8. Bundle `CodeWriter.app.tar.gz` and produce `CodeWriter.app.tar.gz.sig` using the Tauri updater key.
...
- Three assets are attached: `CodeWriter_<version>_aarch64.dmg`, `CodeWriter.app.tar.gz`, `latest.json`.
